/**
 * The site's URL tables: clean-URL rewrites for the static pages in public/, the
 * redirects that give each page one address, and the old site's addresses.
 *
 * Kept out of next.config.mjs so the build config stays small; Website Lab refuses
 * to build a next.config over 4 KB, the shape of the injected loader it screens for.
 */
import fs from 'fs';
