 ...
 * place. The aggregate (rating and total count) is for local archive and
 * prose only — /reviews no longer marks it up, because the live page is the
 * Vexur widget. For the individual reviews the script MERGES — anything
 * Google returns is added or updated, and reviews already in the file are
 * kept rather than dropped. That way the captured set survives, and grows.
 *
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode !== 200)
            return reject(
              new Error("HTTP " + res.statusCode + ": " + text.slice(0, 300)),
            );
          try {
...
async function run() {
  ...
  if (!place) {
    console.error(
      "No matching place. Check BUSINESS and ADDRESS at the top of this script.",
    );
    process.exit(1);
  }
  console.log(
  ...
  for (const review of details.reviews || []) {
    const body = clean(
      review.originalText ? review.originalText.text : (review.text || {}).text,
    );
    if (!body) continue;
    const author =
      clean((review.authorAttribution || {}).displayName) || "Anonymous";
  ...
  const out = {
    ...
    reviews: [...merged.values()].sort((a, b) =>
  ...
  console.log(
    "aggregate  : " +
      out.aggregate.ratingValue +
      " from " +
      out.aggregate.reviewCount +
      " reviews",
  );
  console.log(
    "reviews    : " + out.reviews.length + " (" + added + " new this run)",
  );
