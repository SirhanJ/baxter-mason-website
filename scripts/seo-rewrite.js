...
function readPage(file) {
  ...
  return {
    ...
    description: lib.unesc(
      grab(/<meta name="description" content="([^"]*)"/i).trim(),
    ),
...
const SUBURBS = pages
  ...
  .map((p) => ({
    slug: p.slug,
    name: p.h1.replace(/\s*Buyers Agent$/i, "").trim(),
    url: p.url,
  }))
  .filter((s) => s.name)
...
function breadcrumb(page) {
  ...
  if (page.type === "suburb") {
    ...
    trail.push({
      name: page.h1 || page.title.split("|")[0].trim(),
...
function graphFor(page) {
  ...
  if (page.image)
    webPage.primaryImageOfPage = { "@type": "ImageObject", url: page.image };
  nodes.push(webPage);
...
function dropLegacyOrgBlock(html) {
  return html.replace(
    ...
    (block, offset) => {
      ...
      const insideGraph =
        graphStart !== -1 && offset > graphStart && offset < graphEnd;
...
const { REVIEWS_MARK, homeReviewsBlock } = require("./vexur-reviews-embed");

/**
 * Live Vexur Google Reviews iframe. Replaces the old GHL widget and the later
 * static HTML carousel so a later SEO pass cannot restore either.
 */
function replaceHomeReviews(html, page) {
  if (page.slug !== "index") return html;

  const block = homeReviewsBlock();
  const startMark = html.indexOf(REVIEWS_MARK);
  if (startMark !== -1) {
    const endMark =
      html.indexOf(REVIEWS_MARK, startMark + REVIEWS_MARK.length) +
      REVIEWS_MARK.length;
    return html.slice(0, startMark) + block + html.slice(endMark);
  }
  ...
  out = out.replace(
    /\s*<script src="https:\/\/apisystem\.tech\/js\/reviews_widget\.js"><\/script>/i,
    "",
  );
  return out;
}
...
function addNearbyBlock(html, page) {
  ...
  const neighbours = (NEIGHBOURS[page.slug] || []).filter((slug) =>
    bySlug.has(slug),
  );
  ...
  const links = neighbours
    .map((slug) => {
      ...
      return (
        '<li><a href="' + near.url + '">' + label + " buyers agent</a></li>"
      );
    })
    .join("\n");
  ...
  if (start !== -1) {
    const end =
      html.indexOf(NEARBY_MARK, start + NEARBY_MARK.length) +
      NEARBY_MARK.length;
    return html.slice(0, start) + block + html.slice(end);
  }
...
function addAreasHub(html, page) {
  ...
  if (start !== -1) {
    const end =
      html.indexOf(AREAS_MARK, start + AREAS_MARK.length) + AREAS_MARK.length;
    return html.slice(0, start) + block + html.slice(end);
  }
...
function applyOverrides(html, page) {
  ...
  if (over.description) {
    const escaped = over.description
      .replace(/&/g, "&amp;")
      .replace(/"/g, "&quot;");
    out = out.replace(
